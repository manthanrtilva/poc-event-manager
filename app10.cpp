...
class ManagerA : public IManager<Event::UPtr> {
  PipedEvent txEvent{};
  atomic_queue::AtomicQueueB2<Event::UPtr> txQueue{MAX_QUEUE_SIZE};
...
public:
  ...
  virtual void _runImpl() override {
    ...
    if(txQueue.try_push(std::move(event))) {
  ...
  virtual std::list<Event::UPtr> Produce() override {
    ...
    txEvent.Consume();
    Event::UPtr event;
    while (txQueue.was_empty() == false) {
      events.emplace_back(std::move(txQueue.pop()));
    }
    return events;
  }
