...
public:
  std::int32_t GetSources() override { return txEvent.GetSource(); }
  virtual void _runImpl() override {
    auto event = std::make_unique<EventB>(random_int_in_range(0, 100));
    std::lock_guard<std::mutex> lock(txMutex);
    txQueue.emplace_back(
...
int main() {
  ...
  EventPoller<IProducer<Event::UPtr>::WPtr, IConsumer<Event::UPtr>::WPtr>
      poller;
  poller.Add(EPOLLIN, managerA->GetSources(), managerA, managerB);
  poller.Add(EPOLLIN, managerB->GetSources(), managerB, managerA);
