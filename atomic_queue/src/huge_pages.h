...
public:
  ...
  struct Deleter {
    ...
    template <class T> void operator()(T *p) const { hp_->destroy(p); }
  };
  ...
  bool empty() const noexcept { return cur_ == beg_; }

  size_t available() const noexcept { return end_ - cur_; }

  size_t capacity() const noexcept { return end_ - beg_; }

  size_t used() const noexcept { return cur_ - beg_; }
  ...
  std::unique_ptr<T, Deleter> create_unique_ptr(Args &&...args) {
    return std::unique_ptr<T, Deleter>{new (this->allocate(sizeof(T)))
...
template <class T> struct HugePageAllocator : HugePageAllocatorBase {
  ...
  template <class U> HugePageAllocator(HugePageAllocator<U>) noexcept {}

  T *allocate(size_t n) const {
    return static_cast<T *>(hp->allocate(n * sizeof(T)));
  }

  void deallocate(T *p, size_t n) const { hp->deallocate(p, n * sizeof(T)); }

  template <class U> bool operator==(HugePageAllocator<U> b) const {
    return hp == b.hp;
  }
