...
template <int BITS>
ATOMIC_QUEUE_INLINE static constexpr unsigned
remap_index(unsigned index) noexcept {
...
ATOMIC_QUEUE_INLINE static constexpr T &map(T *ATOMIC_QUEUE_RESTRICT elements,
...
ATOMIC_QUEUE_INLINE static constexpr T or_equal(T x, unsigned u,
                                                Args... rest) noexcept {
  return or_equal(or_equal(x, u), rest...);
}

ATOMIC_QUEUE_INLINE static constexpr uint32_t
round_up_to_power_of_2(uint32_t a) noexcept {
  return increment(or_equal(decrement(a), 1, 2, 4, 8, 16));
}

ATOMIC_QUEUE_INLINE static constexpr uint64_t
round_up_to_power_of_2(uint64_t a) noexcept {
  return increment(or_equal(decrement(a), 1, 2, 4, 8, 16, 32));
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

template <class T> constexpr T nil() noexcept {
...
template <class T>
ATOMIC_QUEUE_INLINE static void destroy_n(T *ATOMIC_QUEUE_RESTRICT p,
                                          unsigned n) noexcept {
  for (auto q = p + n; p != q;)
    (p++)->~T();
}
...
template <class T>
ATOMIC_QUEUE_INLINE static void swap_relaxed(std::atomic<T> &a,
...
template <class T>
ATOMIC_QUEUE_INLINE static void copy_relaxed(std::atomic<T> &a,
...
protected:
  ...
  ATOMIC_QUEUE_INLINE static T
  do_pop_atomic(std::atomic<T> &q_element) noexcept {
    if (Derived::spsc_) {
      for (;;) {
  ...
  template <class T, T NIL>
  ATOMIC_QUEUE_INLINE static void
  do_push_atomic(T element, std::atomic<T> &q_element) noexcept {
    assert(element != NIL);
    if (Derived::spsc_) {
      ...
      for (T expected = NIL; ATOMIC_QUEUE_UNLIKELY(
               !q_element.compare_exchange_weak(expected, element, R, X));
           expected = NIL) {
        do
  ...
  template <class T>
  ATOMIC_QUEUE_INLINE static T do_pop_any(std::atomic<unsigned char> &state,
    ...
    if (Derived::spsc_) {
      ...
      for (;;) {
        ...
        if (ATOMIC_QUEUE_LIKELY(
                state.compare_exchange_weak(expected, LOADING, A, X))) {
          T element{std::move(q_element)};
          state.store(EMPTY, R);
          return element;
        }
  ...
  ATOMIC_QUEUE_INLINE static void do_push_any(U &&element,
    ...
    if (Derived::spsc_) {
      ...
      for (;;) {
        ...
        if (ATOMIC_QUEUE_LIKELY(
                state.compare_exchange_weak(expected, STORING, A, X))) {
          q_element = std::forward<U>(element);
          state.store(STORED, R);
          return;
        }
...
public:
  template <class T> ATOMIC_QUEUE_INLINE bool try_push(T &&element) noexcept {
  ...
  template <class T> ATOMIC_QUEUE_INLINE bool try_pop(T &element) noexcept {
  ...
  template <class T> ATOMIC_QUEUE_INLINE void push(T &&element) noexcept {
    unsigned head;
    if (Derived::spsc_) {
      head = head_.load(X);
      head_.store(head + 1, X);
  ...
  ATOMIC_QUEUE_INLINE unsigned was_size() const noexcept {
    ...
    return static_cast<int>(n) < 0
               ? 0
               : n; // Windows headers break std::min/max by default. Do
                    // std::max<int>(n, 0) the hard way here.
  }
...
template <class T, unsigned SIZE, T NIL = details::nil<T>(),
...
class AtomicQueue2 : public AtomicQueueCommon<
  ...
  ATOMIC_QUEUE_INLINE void do_push(U &&element, unsigned head) noexcept {
    ...
    Base::do_push_any(std::forward<U>(element), states_[index],
                      elements_[index]);
  }
...
template <class T, class A = std::allocator<T>, T NIL = details::nil<T>(),
          bool MAXIMIZE_THROUGHPUT = true, bool TOTAL_ORDER = false,
          bool SPSC = false>
class AtomicQueueB
...
public:
  ...
  AtomicQueueB(AtomicQueueB &&b) noexcept
      : AllocatorElements(static_cast<AllocatorElements &&>(
        ...
        ,
        Base(static_cast<Base &&>(b)), size_(std::exchange(b.size_, 0)),
  ...
  friend void swap(AtomicQueueB &a, AtomicQueueB &b) noexcept { a.swap(b); }
};
...
class AtomicQueueB2
  ...
  template <class U> U *allocate_() {
    U *p = reinterpret_cast<U *>(StorageAllocator::allocate(size_ * sizeof(U)));
    assert(reinterpret_cast<uintptr_t>(p) % alignof(U) ==
           0); // Allocated storage must be suitably aligned for U.
    return p;
  }

  template <class U> void deallocate_(U *p) noexcept {
...
public:
  ...
  AtomicQueueB2(unsigned size, A const &allocator = A{})
      : StorageAllocator(allocator),
        size_(std::max(details::round_up_to_power_of_2(size),
                       1u << (SHUFFLE_BITS * 2))),
        states_(allocate_<AtomicState>()), elements_(allocate_<T>()) {
...
template <class Queue, size_t Capacity> struct CapacityArgAdaptor : Queue {
  CapacityArgAdaptor() : Queue(Capacity) {}
};
